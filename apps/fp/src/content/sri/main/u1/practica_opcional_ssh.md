...
* Desactiva el acceso por SSH del usuario `root` (`PermitRootLogin no`).
* Desactiva por completo la autenticación por contraseña, dejando solo el acceso por clave pública (`PasswordAuthentication no`).
...
* Ajusta los parámetros de baneo (`maxretry`, `bantime`, `findtime`) a valores bajos para poder hacer la demostración en poco tiempo (por ejemplo, 3 intentos fallidos en 1 minuto banean durante 5 minutos).
* Provoca varios intentos fallidos de conexión desde otra máquina del escenario y comprueba que la IP queda baneada.
* Comprueba con `fail2ban-client status sshd` que la IP aparece en la lista de baneadas, y que tras el `bantime` vuelve a poder conectar.
...
2. El momento en que `fail2ban` banea la IP (por ejemplo, `fail2ban-client status sshd` mostrando la IP en la lista de baneadas).
